...
#define ITERATE_TLV_BEGIN(start_ptr, type, length, tlv_ptr, buffer_size) \
    for (unsigned char *iter_ptr = (start_ptr); iter_ptr < (start_ptr) + (buffer_size);) { \
        type = *iter_ptr; \
        iter_ptr++; \
        length = *iter_ptr; \
        iter_ptr++; \
...
decode_tlv_buffer(unsigned char *tlv_buffer, 
    ...
    ITERATE_TLV_BEGIN(tlv_buffer, tlv_type, tlv_len, tlv_value, tlv_buffer_size){
        ...
        switch(tlv_type){
            ...
            case IP_ADDRESS_TLV_CODE:
            for( i = 0; i < units; i++){
                printf("TLV Type = %d, IP Address = %u\n", tlv_type,
                    *(uint32_t *)(tlv_value + (i * get_unit_data_size(tlv_type))));
            }
            break;
...
// ############################################
    ...
    temp += TLV_OVERHEAD_SIZE + 24; // Move temp to the end of the previous TLV

    *temp = IP_ADDRESS_TLV_CODE;
    *(temp + 1) = 66; // Length for the new TLV (4 units, 16 bytes each)

    uint32_t *data_ptr = (uint32_t *)(temp + TLV_OVERHEAD_SIZE);
        *(data_ptr + 0) = 2046824449;
        *(data_ptr + 1) = 2046824450;
        *(data_ptr + 2) = 2046824451;
        *(data_ptr + 3) = 2046824452;
